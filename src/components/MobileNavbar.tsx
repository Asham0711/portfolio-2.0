      ...
      <div className="py-2 px-6 flex justify-between items-center shadow-md dark:shadow-gray-500 fixed top-0 right-0 left-0 bg-[#F5EFFF] dark:bg-[#121212] z-50">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={toggleSidebar} className="flex items-center">
            {isSidebarOpen ? (
              <HiX className="text-2xl text-brand-300" />
            ) : (
              <HiMenuAlt3 className="text-2xl text-brand-300" />
            )}
          </button>
        </div>
      </div>
        ...
        <div className="flex flex-col items-start p-6 space-y-6">
          {navLink.map((link) => (
            <Link
              key={link.title}
              href={link.path}
              className="text-lg text-gray-700 dark:text-gray-300 hover:text-brand-300 transition-all duration-300"
